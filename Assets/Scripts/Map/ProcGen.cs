...
sealed class ProcGen
{
    ...
    private bool SetWallTileIfEmpty(Vector3Int pos)
    {
        ...
        else
        {
            ...
            newTile.sprite = MapManager.instance.WallBgTile.sprite;
            newTile.color = ColorNear(MapManager.instance.WallBgTile.color);
            MapManager.instance.ObstacleMap.SetTile(pos, MapManager.instance.WallTile);
            MapManager.instance.BackgroundMap.SetTile(pos, newTile);
            return false;
        }
    }
    ...
    private void SetFloorTile(Vector3Int pos)
    {
        if (MapManager.instance.ObstacleMap.GetTile(pos))
        {
            ...
            MapManager.instance.BackgroundMap.SetTile(pos, null);
        }
        var newTile = ScriptableObject.CreateInstance<Tile>();
        newTile.sprite = MapManager.instance.FloorTile.sprite;
        newTile.color = ColorNear(MapManager.instance.FloorTile.color);
        MapManager.instance.FloorMap.SetTile(pos, newTile);
    }

    private static Color ColorNear(Color color)
    {
        float multiplier = UnityRandom.Range(0.9f, 1.1f);
        return new Color(
    ...
    private void PlaceEntities(RectangularRoom newRoom, int floorNumber)
    {
        ...
        foreach (string entityName in monsterNames)
        {
